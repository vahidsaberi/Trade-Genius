...
resource "aws_ecs_service" "api_ecs_service" {
  ...
  desired_count   = 1
  load_balancer {
    target_group_arn = aws_lb_target_group.trade_genius_api_tg.arn
  ...
  container_definitions = jsonencode([
    {
      ...
      environment : [
        ...
        { "name" : "DatabaseSettings__ConnectionString", "value" : "Host=${aws_db_instance.postgres.endpoint};Port=5432;Database=trade-genius-db;Username=${var.pg_username};Password=${var.pg_password};Include Error Detail=true" },
        { "name" : "DatabaseSettings__DBProvider", "value" : "postgresql" },
        { "name" : "HangfireSettings__Storage__ConnectionString", "value" : "Host=${aws_db_instance.postgres.endpoint};Port=5432;Database=trade-genius-db;Username=${var.pg_username};Password=${var.pg_password};Include Error Detail=true" },
      ...
      logConfiguration : {
        ...
        "options" : {
          ...
          "awslogs-group" : "trade-genius/dotnet-webapi",
          "awslogs-stream-prefix" : "trade-genius-api"
        }
      },
      portMappings : [
        {
...
resource "aws_cloudwatch_log_group" "api_log_group" {
  name              = "trade-genius/dotnet-webapi"
  retention_in_days = 5
}
