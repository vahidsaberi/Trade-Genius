...
{
    private readonly IMultiTenantStore<TradeGeniusTenantInfo> _tenantStore;
    ...
    public TenantService(
        IMultiTenantStore<TradeGeniusTenantInfo> tenantStore,
    ...
    public async Task<string> CreateAsync(CreateTenantRequest request, CancellationToken cancellationToken)
    {
        ...
        var tenant = new TradeGeniusTenantInfo(request.Id, request.Name, request.ConnectionString, request.AdminEmail, request.Issuer);
    ...
    private async Task<TradeGeniusTenantInfo> GetTenantInfoAsync(string id) =>
        await _tenantStore.TryGetAsync(id)
            ?? throw new NotFoundException(_t["{0} {1} Not Found.", typeof(TradeGeniusTenantInfo).Name, id]);
}